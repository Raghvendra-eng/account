...
public class AllControlers {
    ...
    public ResponseEntity<String> debit(@PathVariable @Min(1) Long accountNumber, @RequestParam @Min(1) Long amount) {
    ...
    public ResponseEntity<String> credit(@PathVariable @Min(1) Long accountNumber, @RequestParam @Min(1) Long amount) {
        User user = accountService.getUser(accountNumber);
        user.incrementAccountBalance(amount) ;
        Transactions newTransaction =
